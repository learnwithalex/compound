...
export function buildDigestHtml(brief: Brief, metrics: PortfolioMetrics): string {
  const stamp = new Date(brief.generatedAt).toLocaleString("en-US", {
    ...
    <tr><td align="center">
      <table width="520" cellpadding="0" cellspacing="0" style="max-width:520px;width:100%;">
        ...
        <tr><td style="padding-bottom:24px;">
          <table cellpadding="0" cellspacing="0"><tr>
            <td style="vertical-align:middle;">
              <img src="${base}/logo-mark.png" width="28" height="24" alt="Compound" style="display:block;"/>
            </td>
            <td style="vertical-align:middle;padding-left:8px;">
              <span style="font-size:16px;font-weight:700;color:#1a1a1a;letter-spacing:-0.02em;">compound</span><span style="font-size:16px;font-weight:700;color:#ff5c00;">.</span>
            </td>
            <td style="vertical-align:middle;padding-left:10px;">
              <span style="font-size:11px;color:#9a9a9a;">Weekly briefing · ${stamp}</span>
            </td>
          </tr></table>
        </td></tr>

        <!-- Hero -->
        <tr><td style="background:#fff;border:1px solid #ebebeb;border-radius:6px;padding:24px 28px;">
        ...
        <tr><td style="text-align:center;padding:0 0 8px;">
          <a href="${base}/app" style="font-size:12px;color:#5e6ad2;text-decoration:none;">Open compound →</a>
          <span style="margin:0 8px;color:#d0d0d0;">·</span>
          <a href="${base}/app/settings" style="font-size:12px;color:#9a9a9a;text-decoration:none;">Manage digest settings</a>
        </td></tr>
...
export async function sendDigest(user: { id: string; email: string }): Promise<{ ok: boolean; error?: string }> {
  const [{ portfolioMetrics }, { generateBrief }] = await Promise.all([
    import("@/lib/metrics"),
    import("@/lib/brief"),
  ]);

  const metrics = await portfolioMetrics(user.id);
  if (metrics.products.length === 0) return { ok: false, error: "no_data" };
  ...
  return res.ok ? { ok: true } : { ok: false, error: `send_failed_${res.status}` };
}
